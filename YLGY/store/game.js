...
const useGame = () => {
  ...
  // style
  let levelBoardStyle = ref({width: '', height: ''});
  // 各层块
  ...
  const initGame = () => {
	...
	levelBoardStyle.value.width = (widthUnit * boxWidthNum + "px");
	levelBoardStyle.value.height = (heightUnit * boxHeightNum + "px");
	
	// levelBoardStyle.value.width = ("400px");
	// levelBoardStyle.value.height = ("400px");
    // 1. 规划块数
    // 块数单位（总块数必须是该值的倍数）
